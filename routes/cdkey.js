    ...
    ref.once("value", function(codeState) {
        if (codeState.val() == null) {
            ...
            if (codeState.val()["new"] == true) {
                ...
                ref.update({"deviceCode" : req.params.deviceCode, "expireTime" : validTime, "new" : false}, function(error) {
                    if (error) {
                        res.send("错误，请再试一次!Error, Please try again!");
                    } else {
                        res.send("true");
            ...
            } else if (codeState.val()["deviceCode"] == req.params.deviceCode && codeState.val()["expireTime"] > timeSeconds) {
                res.send("true");
            } else {
                if (codeState.val()["expireTime"] < timeSeconds) {
                    ...
                    ref.update({"deviceCode" : req.params.deviceCode, "expireTime" : updateValidTime}, function(error) {
                        if (error) {
                            res.send("错误，请再试一次!Error, Please try again!");
                        } else {
                           res.send("true");
